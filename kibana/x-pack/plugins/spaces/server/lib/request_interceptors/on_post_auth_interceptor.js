...
function initSpacesOnPostAuthRequestInterceptor({
  ...
  http.registerOnPostAuth(async (request, response, toolkit) => {
    ...
    const spacesClient = await spacesService.scopedClient(request); // if requesting the application root, then show the Space Selector UI to allow the user to choose which space
    ...
    if (isRequestingApplication) {
      ...
      try {
        ...
        const statusCode = wrappedError.statusCode; // If user is not authorized, or the space cannot be found, allow them to select another space
