 ...
 */
var ES_INTERVAL_STRING_REGEX = new RegExp('^([1-9][0-9]*)\\s*(' + _datemath.default.units.join('|') + ')$');
...
function parseEsInterval(interval) {
  var matches = String(interval).trim().match(ES_INTERVAL_STRING_REGEX);
  ...
  var unit = matches[2];
  var type = _datemath.default.unitsMap[unit].type;
