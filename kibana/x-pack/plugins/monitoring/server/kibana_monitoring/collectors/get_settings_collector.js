...
async function checkForEmailValue(config, _shouldUseNull = shouldUseNull, _getDefaultAdminEmail = getDefaultAdminEmail) {
  const defaultAdminEmail = await _getDefaultAdminEmail(config); // Allow null so clearing the advanced setting will be reflected in the data
  ...
  return usageCollection.makeStatsCollector({
    ...
    async fetch() {
      ...
      if (defaultAdminEmail || defaultAdminEmail === null && shouldUseNull) {
        ...
        this.log.debug(`[${defaultAdminEmail}] default admin email setting found, sending [${_constants.KIBANA_SETTINGS_TYPE}] monitoring document.`);
      } else {
        this.log.debug(`not sending [${_constants.KIBANA_SETTINGS_TYPE}] monitoring document because [${defaultAdminEmail}] is null or invalid.`);
