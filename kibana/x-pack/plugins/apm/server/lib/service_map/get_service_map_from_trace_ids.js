...
async function getServiceMapFromTraceIds({
  ...
  const scriptResponse = (_serviceMapResponse$a = serviceMapResponse.aggregations) === null || _serviceMapResponse$a === void 0 ? void 0 : _serviceMapResponse$a.service_map.value;
  ...
  if (serviceName || environment) {
    paths = paths.filter(path => {
      return path.some(node => {
        ...
        if (serviceName) {
          matches = matches && _elasticsearch_fieldnames.SERVICE_NAME in node && node[_elasticsearch_fieldnames.SERVICE_NAME] === serviceName;
        }

        if (environment) {
          matches = matches && _elasticsearch_fieldnames.SERVICE_ENVIRONMENT in node && node[_elasticsearch_fieldnames.SERVICE_ENVIRONMENT] === environment;
        }
  ...
  const connections = (0, _lodash.uniq)(paths.flatMap(path => {
    ...
    return (0, _lodash.find)(array, value);
  });
  return {
    connections,
