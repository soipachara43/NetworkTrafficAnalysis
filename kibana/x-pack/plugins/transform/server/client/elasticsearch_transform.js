...
const elasticsearchJsPlugin = (Client, config, components) => {
  ...
  const transform = Client.prototype.transform.prototype; // Currently the endpoint uses a default size of 100 unless a size is supplied.
  ...
  transform.stopTransform = ca({
    urls: [{
      fmt: '/_transform/<%=transformId%>/_stop?&force=<%=force%>&wait_for_completion=<%waitForCompletion%>',
      req: {
        transformId: {
          type: 'string'
        },
