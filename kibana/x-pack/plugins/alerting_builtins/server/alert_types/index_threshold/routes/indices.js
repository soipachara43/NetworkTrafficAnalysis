...
function createIndicesRoute(service, router, baseRoute) {
  ...
  async function handler(ctx, req, res) {
    ...
    try {
      ...
      service.logger.warn(`route ${path} error getting aliases from pattern "${pattern}": ${err.message}`);
    }
    ...
    try {
      ...
      service.logger.warn(`route ${path} error getting indices from pattern "${pattern}": ${err.message}`);
    }
