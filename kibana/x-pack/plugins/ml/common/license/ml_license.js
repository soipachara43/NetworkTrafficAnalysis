...
class MlLicense {
  ...
  setup(license$, postInitFunctions) {
    this._licenseSubscription = license$.subscribe(async license => {
      ...
      this._isMinimumLicense = this._license.check(_app.PLUGIN_ID, MINIMUM_LICENSE).state === 'valid';
      this._isFullLicense = this._license.check(_app.PLUGIN_ID, MINIMUM_FULL_LICENSE).state === 'valid';
