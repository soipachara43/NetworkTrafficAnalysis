...
function getAllFilters(context) {
  const mgr = new _filter.FilterManager(context.ml.mlClient.callAsCurrentUser);
  return mgr.getAllFilters();
}

function getAllFilterStats(context) {
  const mgr = new _filter.FilterManager(context.ml.mlClient.callAsCurrentUser);
  return mgr.getAllFilterStats();
}

function getFilter(context, filterId) {
  const mgr = new _filter.FilterManager(context.ml.mlClient.callAsCurrentUser);
  return mgr.getFilter(filterId);
}

function newFilter(context, filter) {
  const mgr = new _filter.FilterManager(context.ml.mlClient.callAsCurrentUser);
  return mgr.newFilter(filter);
}

function updateFilter(context, filterId, filter) {
  const mgr = new _filter.FilterManager(context.ml.mlClient.callAsCurrentUser);
  return mgr.updateFilter(filterId, filter);
}

function deleteFilter(context, filterId) {
  const mgr = new _filter.FilterManager(context.ml.mlClient.callAsCurrentUser);
...
function filtersRoutes({
  ...
  router.put({
    path: '/api/ml/filters',
    validate: {
      body: _configSchema.schema.object(_filters_schema.createFilterSchema)
    }
  }, mlLicense.fullLicenseAPIGuard(async (context, request, response) => {
    try {
  ...
  router.put({
    ...
    validate: {
      ...
      body: _configSchema.schema.object(_filters_schema.updateFilterSchema)
    }
  }, mlLicense.fullLicenseAPIGuard(async (context, request, response) => {
    try {
      const {
