 ...
 */
function importDataProvider(callAsCurrentUser) {
  ...
  async function createIndex(index, settings, mappings) {
    const body = {
      mappings: {
        _meta: {
          created_by: _file_datavisualizer.INDEX_META_DATA_CREATED_BY
        },
        properties: mappings
      }
    };
    ...
    await callAsCurrentUser('indices.create', {
      index,
      body
    });
  }
  ...
  async function createPipeline(id, pipeline) {
    return await callAsCurrentUser('ingest.putPipeline', {
      id,
      body: pipeline
    });
  }
