...
function resultsServiceProvider(callAsCurrentUser) {
  ...
  async function getAnomaliesTableData(jobIds, criteriaFields, influencers, aggregationInterval, threshold, earliestMs, latestMs, dateFormatTz, maxRecords = _search.ANOMALIES_TABLE_DEFAULT_QUERY_SIZE, maxExamples = DEFAULT_MAX_EXAMPLES, influencersFilterQuery) {
    ...
    if (resp.hits.total !== 0) {
      ...
      records = _lodash.default.sortBy(records, 'timestamp');
      ...
      if (categoryAnomalies.length > 0) {
        ...
        categoryAnomalies.forEach(anomaly => {
          if (!_lodash.default.has(categoryIdsByJobId, anomaly.jobId)) {
  ...
  async function getMaxAnomalyScore(jobIds = [], earliestMs, latestMs) {
    ...
    const maxScore = _lodash.default.get(resp, ['aggregations', 'max_score', 'value'], null);
  ...
  async function getLatestBucketTimestampByJob(jobIds = []) {
    ...
    const bucketsByJobId = _lodash.default.get(resp, ['aggregations', 'byJobId', 'buckets'], []);
  ...
  async function getCategoryExamples(jobId, categoryIds, maxExamples) {
    ...
    if (resp.hits.total !== 0) {
      resp.hits.hits.forEach(hit => {
        if (maxExamples) {
          examplesByCategoryId[hit._source.category_id] = _lodash.default.slice(hit._source.examples, 0, Math.min(hit._source.examples.length, maxExamples));
        } else {
  ...
  return {
    ...
    getMaxAnomalyScore,
    getPartitionFieldsValues: (0, _get_partition_fields_values.getPartitionFieldsValuesFactory)(callAsCurrentUser)
  };
}