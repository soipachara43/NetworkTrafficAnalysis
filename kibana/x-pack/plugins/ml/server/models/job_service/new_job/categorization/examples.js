...
function categorizationExamplesProvider(callWithRequest, callWithInternalUser) {
  const validationResults = new _validation_results.ValidationResults();

  async function categorizationExamples(indexPatternTitle, query, size, categorizationFieldName, timeField, start, end, analyzer) {
    ...
    const tempExamples = results.hits.hits.map(({
      _source
    }) => _source[categorizationFieldName]);
  ...
  async function loadTokens(examples, analyzer) {
    const {
      tokens
    } = await callWithInternalUser('indices.analyze', {
  ...
  async function validateCategoryExamples(indexPatternTitle, query, size, categorizationFieldName, timeField, start, end, analyzer) {
    const resp = await categorizationExamples(indexPatternTitle, query, _categorization_job.CATEGORY_EXAMPLES_SAMPLE_SIZE, categorizationFieldName, timeField, start, end, analyzer);
    const {
      examples
    } = resp;
