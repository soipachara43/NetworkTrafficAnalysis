  ...
  if (Array.isArray(value)) {
    if (value.length === 1) {
      ...
      var values = value.map(function (val) {
        return formatSingleValue(val, mlFunction, undefined, record);
      });
...
function formatSingleValue(value, mlFunction, fieldFormat, record) {
  ...
  if (mlFunction === 'time_of_week') {
    ...
    var utcMoment = _moment.default.utc(d).startOf('week').add(value, 's');

    return (0, _moment.default)(utcMoment.valueOf()).format('ddd HH:mm');
  } else if (mlFunction === 'time_of_day') {
    var _d = record !== undefined && record.timestamp !== undefined ? new Date(record.timestamp) : new Date();

    var _utcMoment = _moment.default.utc(_d).startOf('day').add(value, 's');

    return (0, _moment.default)(_utcMoment.valueOf()).format('HH:mm');
  } else {
