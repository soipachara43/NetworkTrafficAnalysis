...
exports.checkGetManagementMlJobs = checkGetManagementMlJobs;
...
var _management_urls = require("../management/management_urls");
 ...
 */
var privileges = (0, _privileges.getDefaultPrivileges)(); // manage_ml requires all monitor and admin cluster privileges: https://github.com/elastic/elasticsearch/blob/664a29c8905d8ce9ba8c18aa1ed5c5de93a0eabc/x-pack/plugin/core/src/main/java/org/elasticsearch/xpack/core/security/authz/privilege/ClusterPrivilege.java#L53

function checkGetManagementMlJobs() {
  return new Promise(function (resolve, reject) {
    (0, _get_privileges.getManageMlPrivileges)().then(function (_ref) {
      var capabilities = _ref.capabilities,
          isPlatinumOrTrialLicense = _ref.isPlatinumOrTrialLicense,
          mlFeatureEnabledInSpace = _ref.mlFeatureEnabledInSpace;
      privileges = capabilities; // Loop through all privileges to ensure they are all set to true.

      var isManageML = Object.values(privileges).every(function (p) {
        return p === true;
      });

      if (isManageML === true && isPlatinumOrTrialLicense === true) {
        return resolve({
          mlFeatureEnabledInSpace: mlFeatureEnabledInSpace
        });
      } else {
        window.location.href = _management_urls.ACCESS_DENIED_PATH;
        return reject();
      }
    });
  });
...
function checkGetJobsPrivilege() {
  return new Promise(function (resolve, reject) {
    (0, _get_privileges.getPrivileges)().then(function (_ref2) {
      var capabilities = _ref2.capabilities,
          isPlatinumOrTrialLicense = _ref2.isPlatinumOrTrialLicense;
      privileges = capabilities; // the minimum privilege for using ML with a platinum or trial license is being able to get the transforms list.
...
function checkCreateJobsPrivilege() {
  return new Promise(function (resolve, reject) {
    (0, _get_privileges.getPrivileges)().then(function (_ref3) {
      var capabilities = _ref3.capabilities,
          isPlatinumOrTrialLicense = _ref3.isPlatinumOrTrialLicense;
      privileges = capabilities; // if the license is basic (isPlatinumOrTrialLicense === false) then do not redirect,
      // allow the promise to resolve as the separate license check will redirect then user to
      // a basic feature
...
function checkFindFileStructurePrivilege() {
  return new Promise(function (resolve, reject) {
    (0, _get_privileges.getPrivileges)().then(function (_ref4) {
      var capabilities = _ref4.capabilities;
      privileges = capabilities; // the minimum privilege for using ML with a basic license is being able to use the datavisualizer.
      // all other functionality is controlled by the return privileges object
...
function checkPermission(privilegeType) {
  var licenseHasExpired = (0, _license.hasLicenseExpired)();
