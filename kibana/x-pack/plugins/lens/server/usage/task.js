...
async function getDailyEvents(kibanaIndex, callCluster) {
  const aggs = {
    daily: {
      date_histogram: {
  ...
  return ({
    ...
    return {
      async run() {
        ...
        return Promise.all([getDailyEvents(kibanaIndex, callCluster), (0, _visualization_counts.getVisualizationCounts)(callCluster, kibanaIndex)]).then(([lensTelemetry, lensVisualizations]) => {
          return {
            state: {
              runs: (state.runs || 0) + 1,
