...
const initTestBed = (0, _test_utils.registerTestBed)((0, _setup_environment.WithAppDependencies)(_home.SnapshotRestoreHome), testBedConfig);
...
const setup = async () => {
  ...
  const clickRepositoryAt = async index => {
    ...
    const repositoryLink = (0, _test_utils.findTestSubject)(rows[index].reactWrapper, 'repositoryLink');
  ...
  const clickRepositoryActionAt = async (index, action) => {
    ...
    const button = (0, _test_utils.findTestSubject)(lastColumn, `${action}RepositoryButton`);
    await (0, _testUtils.act)(async () => {
  ...
  const clickSnapshotAt = async index => {
    ...
    const snapshotLink = (0, _test_utils.findTestSubject)(rows[index].reactWrapper, 'snapshotLink');
    await (0, _testUtils.act)(async () => {
      const {
        href
      } = snapshotLink.props();
