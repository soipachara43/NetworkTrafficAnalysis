...
function telemetryTaskRunner(logger, core, kibanaIndex) {
  return ({
    ...
    return {
      async run() {
        return Promise.all([(0, _actions_telemetry.getTotalCount)(callCluster, kibanaIndex), (0, _actions_telemetry.getInUseTotalCount)(callCluster, kibanaIndex)]).then(([totalAggegations, countActiveTotal]) => {
          return {
            state: {
              runs: (state.runs || 0) + 1,
