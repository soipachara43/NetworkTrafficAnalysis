 ...
 */
async function getTotalCount(callCluster, kibanaIndex) {
  ...
  return {
    countTotal: Object.keys(searchResult.aggregations.byActionTypeId.value.types).reduce((total, key) => parseInt(searchResult.aggregations.byActionTypeId.value.types[key], 0) + total, 0),
    countByType: Object.keys(searchResult.aggregations.byActionTypeId.value.types).reduce((obj, key) => ({ ...obj,
      [key.replace('.', '__')]: searchResult.aggregations.byActionTypeId.value.types[key]
    }), {})
  };
}

async function getInUseTotalCount(callCluster, kibanaIndex) {
  const scriptedMetric = {
    scripted_metric: {
