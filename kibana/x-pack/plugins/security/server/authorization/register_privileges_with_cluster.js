...
async function registerPrivilegesWithCluster(logger, privileges, application, clusterClient) {
  const arePrivilegesEqual = (existingPrivileges, expectedPrivileges) => {
    ...
    return (0, _lodash.isEqual)(existingPrivileges, expectedPrivileges, (value, other, key) => {
  ...
  try {
    ...
    for (const privilegeToDelete of privilegesToDelete) {
      logger.debug(`Deleting Kibana Privilege ${privilegeToDelete} from Elasticearch for ${application}`);
    ...
    await clusterClient.callAsInternalUser('shield.postPrivileges', {
