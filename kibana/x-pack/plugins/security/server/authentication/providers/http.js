...
class HTTPAuthenticationProvider extends _base.BaseAuthenticationProvider {
  ...
  constructor(options, httpOptions) {
    var _ref, _httpOptions$supporte;
    ...
    if (((_ref = httpOptions === null || httpOptions === void 0 ? void 0 : (_httpOptions$supporte = httpOptions.supportedSchemes) === null || _httpOptions$supporte === void 0 ? void 0 : _httpOptions$supporte.size) !== null && _ref !== void 0 ? _ref : 0) === 0) {
  ...
  async authenticate(request) {
    ...
    try {
      ...
      this.logger.debug(`Request to ${request.url.path} has been authenticated via authorization header with "${authorizationHeader.scheme}" scheme.`);
      return _authentication_result.AuthenticationResult.succeeded(user);
    } catch (err) {
      this.logger.debug(`Failed to authenticate request to ${request.url.path} via authorization header with "${authorizationHeader.scheme}" scheme: ${err.message}`);
