  ...
  async invalidate({
    ...
    if (refreshToken) {
      ...
      try {
        ...
        this.logger.debug(`Failed to invalidate refresh token: ${err.message}`); // We don't re-throw the error here to have a chance to invalidate access token if it's provided.
