...
function defineRoleMappingFeatureCheckRoute({
  ...
  router.get({
    ...
    } = await clusterClient.asScoped(request).callAsCurrentUser('shield.hasPrivileges', {
      body: {
...
async function getEnabledRoleMappingsFeatures(clusterClient, logger) {
  logger.debug(`Retrieving role mappings features`);
  const nodeScriptSettingsPromise = clusterClient.callAsInternalUser('nodes.info', {
  ...
  const xpackUsagePromise = clusterClient // `transport.request` is potentially unsafe when combined with untrusted user input.
