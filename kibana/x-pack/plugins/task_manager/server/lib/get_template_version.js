...
function getTemplateVersion(versionStr) {
  ...
  const minorStr = splitted[2] || ''; // pad each part with leading 0 to make 2 characters

  const padded = splitted.map(v => {
    const vMatches = v.match(/\d+/);

    if (vMatches) {
      return (0, _lodash.padLeft)(vMatches[0], 2, '0');
    }

    return '00';
  });
  const [majorV, minorV, patchV] = padded; // append the alpha/beta/rc indicator
  ...
  if (minorStr.match('alpha')) {
    ...
    if (matches != null && matches.groups != null) {
      ...
      buildV = (0, _lodash.padLeft)(`${alphaVerInt}`, 2, '0');
    }
