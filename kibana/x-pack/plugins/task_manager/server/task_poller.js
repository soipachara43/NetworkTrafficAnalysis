...
function createTaskPoller({
  ...
  (0, _rxjs.interval)(pollInterval).pipe((0, _operators.mapTo)(_Option.none))).pipe( // buffer all requests in a single set (to remove duplicates) as we don't want
  ...
  (0, _operators.concatMap)(async set => {
    closeSleepPerf();
    return (0, _result_type.map)((await (0, _result_type.promiseResult)(work(...(0, _pull_from_set.pullFromSet)(set, getCapacity())))), workResult => (0, _result_type.asOk)(workResult), err => {
...
(function (PollingErrorType) {
  ...
  PollingErrorType[PollingErrorType["RequestCapacityReached"] = 1] = "RequestCapacityReached";
