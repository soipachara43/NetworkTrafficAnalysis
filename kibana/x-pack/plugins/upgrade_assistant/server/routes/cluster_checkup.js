...
function registerClusterCheckupRoutes({
  ...
  router.get({
    ...
    try {
      ...
      const status = await (0, _es_migration_apis.getUpgradeAssistantStatus)(dataClient, isCloudEnabled, indexPatterns);
      const callAsCurrentUser = dataClient.callAsCurrentUser.bind(dataClient);
      const reindexActions = (0, _reindex_actions.reindexActionsFactory)(savedObjectsClient, callAsCurrentUser);
