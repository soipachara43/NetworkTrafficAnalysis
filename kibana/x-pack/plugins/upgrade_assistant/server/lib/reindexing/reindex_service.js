...
const reindexServiceFactory = (callAsUser, actions, log, licensing, apmIndexPatterns = []) => {
  // ------ Utility functions
  ...
  const stopWatcher = async () => {
    ...
    await actions.runWhileIndexGroupLocked(_types.IndexGroup.watcher, async watcherDoc => {
      const {
        acknowledged
      } = await callAsUser('transport.request', {
        path: '/_watcher/_stop',
        method: 'POST'
      });
  ...
  const cleanupChanges = async reindexOp => {
    ...
    if (reindexOp.attributes.lastCompletedStep >= _types.ReindexStep.newIndexCreated && reindexOp.attributes.lastCompletedStep < _types.ReindexStep.aliasCreated) {
      await callAsUser('indices.delete', {
  ...
  const startReindexing = async reindexOp => {
    ...
    const indicesState = await (0, _es_indices_state_check.esIndicesStateCheck)(callAsUser, [indexName]);
  ...
  const switchAlias = async reindexOp => {
    ...
    if ((reindexOptions === null || reindexOptions === void 0 ? void 0 : reindexOptions.openAndClose) === true) {
      await callAsUser('indices.close', {
        index: indexName
      });
    }
  ...
  return {
    ...
    async createReindexOperation(indexName, opts) {
      if (isSystemIndex(indexName)) {
        throw _error.error.reindexSystemIndex(`Reindexing system indices are not yet supported within this major version. Upgrade to the latest ${_version.CURRENT_MAJOR_VERSION}.x minor version.`);
      }

      const indexExists = await callAsUser('indices.exists', {
        index: indexName
      });
      ...
      return actions.createReindexOp(indexName, (opts === null || opts === void 0 ? void 0 : opts.enqueue) ? {
        queueSettings: {
          queuedAt: Date.now()
        }
      } : undefined);
    ...
    async resumeReindexOperation(indexName, opts) {
      ...
      return actions.runWhileLocked(reindexOp, async op => {
        ...
        const queueSettings = (opts === null || opts === void 0 ? void 0 : opts.enqueue) ? {
          queuedAt: Date.now()
        } : undefined;
        return actions.updateReindexOp(op, {
    ...
    async startQueuedReindexOperation(indexName) {
      ...
      if (!((_reindexOp$attributes = reindexOp.attributes.reindexOptions) === null || _reindexOp$attributes === void 0 ? void 0 : _reindexOp$attributes.queueSettings)) {
