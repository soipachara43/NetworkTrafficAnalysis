...
const sourceNameForIndex = indexName => {
  ...
  const reindexedMatcher = new RegExp(`(-reindexed-v5$|reindexed-v${_version.PREV_MAJOR_VERSION}-)`, 'g');
...
const generateNewIndexName = indexName => {
  ...
  const currentVersion = `reindexed-v${_version.CURRENT_MAJOR_VERSION}`;
