...
exports.getDeprecatedApmIndices = getDeprecatedApmIndices;
exports.apmReindexScript = exports.isLegacyApmIndex = void 0;
 ...
 */
async function getDeprecatedApmIndices(clusterClient, indexPatterns = []) {
  ...
  return Object.keys(indices).reduce((deprecations, index) => {
...
const isLegacyApmIndex = (indexName, apmIndexPatterns = [], mappings) => {
  const defaultValue = '0.0.0';
  const version = (0, _lodash.get)(mappings, '_meta.version', defaultValue);
  const clientVersion = new _semver.SemVer((0, _semver.valid)(version) ? version : defaultValue);
  ...
  const find = apmIndexPatterns.find(pattern => {
    return (0, _minimatch.default)(indexName, pattern);
  });
  return Boolean(find);
}; // source: https://github.com/elastic/apm-integration-testing/blob/master/tests/server/test_upgrade.py


exports.isLegacyApmIndex = isLegacyApmIndex;
const apmReindexScript = `
  // add ecs version
...
`;
exports.apmReindexScript = apmReindexScript;