...
async function getUpgradeAssistantStatus(dataClient, isCloudEnabled, apmIndices) {
  const [deprecations, apmIndexDeprecations] = await Promise.all([dataClient.callAsCurrentUser('transport.request', {
    path: '/_migration/deprecations',
    method: 'GET'
  }), (0, _apm.getDeprecatedApmIndices)(dataClient, apmIndices)]);
  ...
  if (indexNames.length) {
    const indexStates = await (0, _es_indices_state_check.esIndicesStateCheck)(dataClient.callAsCurrentUser.bind(dataClient), indexNames);
    indices.forEach(indexData => {
      indexData.blockerForReindexing = indexStates[indexData.index] === 'close' ? 'index-closed' : undefined;
    });
  }
