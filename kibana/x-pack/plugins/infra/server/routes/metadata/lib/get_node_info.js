 ...
 */
const getNodeInfo = async (framework, requestContext, sourceConfiguration, nodeId, nodeType) => {
  ...
  if (nodeType === 'pod') {
    const kubernetesNodeName = await (0, _get_pod_node_name.getPodNodeName)(framework, requestContext, sourceConfiguration, nodeId, nodeType);

    if (kubernetesNodeName) {
      return getNodeInfo(framework, requestContext, sourceConfiguration, kubernetesNodeName, 'host');
    }
  ...
  if (!_constants.CLOUD_METRICS_MODULES.some(m => (0, _lodash.startsWith)(nodeType, m))) {
    (0, _lodash.set)(params, 'body.query.bool.must_not', _constants.CLOUD_METRICS_MODULES.map(module => ({
      match: {
        'event.module': module
      }
    })));
