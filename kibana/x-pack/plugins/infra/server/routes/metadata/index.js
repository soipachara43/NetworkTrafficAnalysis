...
const initMetadataRoute = libs => {
  ...
  framework.registerRoute({
    ...
    try {
      ...
      const {
        configuration
      } = await libs.sources.getSourceConfiguration(requestContext, sourceId);
      const metricsMetadata = await (0, _get_metric_metadata.getMetricMetadata)(framework, requestContext, configuration, nodeId, nodeType);
      const metricFeatures = (0, _pick_feature_name.pickFeatureName)(metricsMetadata.buckets).map(nameToFeature('metrics'));
      const info = await (0, _get_node_info.getNodeInfo)(framework, requestContext, configuration, nodeId, nodeType);
      const cloudInstanceId = (0, _lodash.get)(info, 'cloud.instance.id');
      const cloudMetricsMetadata = cloudInstanceId ? await (0, _get_cloud_metric_metadata.getCloudMetricsMetadata)(framework, requestContext, configuration, cloudInstanceId) : {
        buckets: []
      };
