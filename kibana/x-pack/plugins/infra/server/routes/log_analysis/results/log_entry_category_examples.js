...
const initGetLogEntryCategoryExamplesRoute = ({
  ...
  framework.registerRoute({
    ...
    } = (0, _pipeable.pipe)(_log_analysis.getLogEntryCategoryExamplesRequestPayloadRT.decode(request.body), (0, _Either.fold)((0, _runtime_types.throwErrors)(_boom.default.badRequest), _function.identity));

    try {
      const {
        data: logEntryCategoryExamples,
        timing
      } = await logEntryCategoriesAnalysis.getLogEntryCategoryExamples(requestContext, request, sourceId, startTime, endTime, categoryId, exampleCount);
      return response.ok({
