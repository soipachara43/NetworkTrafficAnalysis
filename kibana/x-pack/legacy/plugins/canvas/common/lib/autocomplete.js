...
function getFnArgDefAtPosition(specs, expression, position) {
  try {
    ...
    const fnDef = (0, _expressions.getByAlias)(specs, fn.function);

    if (fnDef && argName) {
      const argDef = (0, _expressions.getByAlias)(fnDef.args, argName);
      return {
        fnDef,
        argDef,
        argStart,
...
function getFnNameSuggestions(specs, ast, fnIndex) {
  ...
  const prevFnDef = prevFn && (0, _expressions.getByAlias)(specs, prevFn.node.function);
  const prevFnType = prevFnDef && prevFnDef.type;
  const nextFnDef = nextFn && (0, _expressions.getByAlias)(specs, nextFn.node.function);
...
function getSubFnNameSuggestions(specs, ast, fnIndex, parentFn, parentFnArgName, contextFn) {
  ...
  const parentFnDef = (0, _expressions.getByAlias)(specs, parentFn);
  const matchingArgDef = (0, _expressions.getByAlias)(parentFnDef.args, parentFnArgName);

  if (!matchingArgDef) {
    return [];
  }

  const contextFnDef = contextFn ? (0, _expressions.getByAlias)(specs, contextFn) : null;
