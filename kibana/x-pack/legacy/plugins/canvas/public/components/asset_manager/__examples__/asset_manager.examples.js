"use strict";

var _addonActions = require("@storybook/addon-actions");

var _react = require("@storybook/react");

var _react2 = _interopRequireDefault(require("react"));

var _asset_manager = require("../asset_manager");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

/*
 * Copyright Elasticsearch B.V. and/or licensed to Elasticsearch B.V. under one
 * or more contributor license agreements. Licensed under the Elastic License;
 * you may not use this file except in compliance with the Elastic License.
 */
var AIRPLANE = {
  '@created': '2018-10-13T16:44:44.648Z',
  id: 'airplane',
  type: 'dataurl',
  value: 'data:image/svg+xml;base64,'
};
var MARKER = {
  '@created': '2018-10-13T16:44:44.648Z',
  id: 'marker',
  type: 'dataurl',
  value: 'data:image/svg+xml;base64,'
};
(0, _react.storiesOf)('components/Assets/AssetManager', module).add('no assets', function () {
  return (// @ts-ignore @types/react has not been updated to support defaultProps yet.
    _react2.default.createElement(_asset_manager.AssetManager, {
      onAddImageElement: (0, _addonActions.action)('onAddImageElement'),
      onAssetAdd: (0, _addonActions.action)('onAssetAdd'),
      onAssetCopy: (0, _addonActions.action)('onAssetCopy'),
      onAssetDelete: (0, _addonActions.action)('onAssetDelete')
    })
  );
}).add('two assets', function () {
  return _react2.default.createElement(_asset_manager.AssetManager, {
    assetValues: [AIRPLANE, MARKER],
    onAddImageElement: (0, _addonActions.action)('onAddImageElement'),
    onAssetAdd: (0, _addonActions.action)('onAssetAdd'),
    onAssetCopy: (0, _addonActions.action)('onAssetCopy'),
    onAssetDelete: (0, _addonActions.action)('onAssetDelete')
  });
});