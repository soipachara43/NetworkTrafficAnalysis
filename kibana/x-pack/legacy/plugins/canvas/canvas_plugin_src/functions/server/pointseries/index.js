...
function pointseries() {
  ...
  return {
    ...
    fn: (input, args) => {
      ...
      const measureKeys = (0, _lodash2.groupBy)(rows, row => dimensions.map(({
        name
      }) => {
      ...
      Object.values(measureKeys).forEach(valueRows => {
        ...
        valueRows.forEach(row => {
          Object.assign(results[row[PRIMARY_KEY]], (0, _lodash2.zipObject)(measureNames, measureValues));
        });
      ...
      const resultingRows = (0, _lodash.default)(Object.values(results).map(row => (0, _lodash2.omit)(row, PRIMARY_KEY)), JSON.stringify);
      return {
        type: 'pointseries',
        columns,
        rows: resultingRows
