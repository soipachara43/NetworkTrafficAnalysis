...
var _find_non_existent_items = require("../utils/find_non_existent_items");
...
class CMBeatsDomain {
  ...
  async update(userOrToken, beatId, beatData) {
    ...
    if (typeof userOrToken === 'string') {
      ...
      } = this.tokens.verifyToken(beat ? beat.access_token : '', userOrToken);
  ...
  async enrollBeat(enrollmentToken, beatId, remoteAddress, beat) {
    const {
      token,
      expires_on
    } = await this.tokens.getEnrollmentToken(enrollmentToken); // eslint-disable-next-line @typescript-eslint/camelcase
  ...
  async removeTagsFromBeats(user, removals) {
    ...
    const nonExistentBeatIds = (0, _find_non_existent_items.findNonExistentItems)(beats, beatIds);
    const nonExistentTags = await (0, _find_non_existent_items.findNonExistentItems)(tags, tagIds);
  ...
  async assignTagsToBeats(user, assignments) {
    ...
    const nonExistentBeatIds = (0, _find_non_existent_items.findNonExistentItems)(beats, beatIds);
    const nonExistentTags = (0, _find_non_existent_items.findNonExistentItems)(tags, tagIds); // FIXME break out back into route / function response
