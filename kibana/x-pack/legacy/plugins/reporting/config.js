...
async function config(Joi) {
  return Joi.object({
    ...
    capture: Joi.object({
      ...
      // deprecated
      concurrency: Joi.number().integer().default(_config.config.concurrency),
      // deprecated
      browser: Joi.object({
        type: Joi.any().valid(_constants.BROWSER_TYPE).default(_constants.BROWSER_TYPE),
        ...
        chromium: Joi.object({
          ...
          disableSandbox: Joi.boolean().default((await (0, _browsers.getDefaultChromiumSandboxDisabled)())),
          proxy: Joi.object({
    ...
    csv: Joi.object({
      ...
      maxSizeBytes: Joi.number().integer().default(1024 * 1024 * 10),
      // bytes in a kB * kB in a mB * 10
      scroll: Joi.object({
        duration: Joi.string().regex(/^[0-9]+(d|h|m|s|ms|micros|nanos)$/, {
          name: 'DurationString'
        }).default('30s'),
