...
const setTimelineEndDate = date => {
  cy.get(_date_picker.DATE_PICKER_END_DATE_POPOVER_BUTTON_TIMELINE).click({
    force: true
  });
  cy.get(_date_picker.DATE_PICKER_ABSOLUTE_TAB).first().click({
    force: true
...
const setTimelineStartDate = date => {
  cy.get(_date_picker.DATE_PICKER_START_DATE_POPOVER_BUTTON_TIMELINE).click({
    force: true
  });
  cy.get(_date_picker.DATE_PICKER_ABSOLUTE_TAB).first().click({
    force: true
...
const updateTimelineDates = () => {
  cy.get(_date_picker.DATE_PICKER_APPLY_BUTTON_TIMELINE).click({
    force: true
  }).invoke('text').should('not.equal', 'Updating');
};
