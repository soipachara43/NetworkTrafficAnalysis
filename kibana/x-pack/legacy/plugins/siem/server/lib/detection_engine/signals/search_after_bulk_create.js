...
const searchAfterAndBulkCreate = async ({
  ...
  toReturn.lastLookBackDate = someResult.hits.hits.length > 0 ? new Date((_someResult$hits$hits = someResult.hits.hits[someResult.hits.hits.length - 1]) === null || _someResult$hits$hits === void 0 ? void 0 : _someResult$hits$hits._source['@timestamp']) : null;
  ...
  const totalHits = typeof someResult.hits.total === 'number' ? someResult.hits.total : someResult.hits.total.value; // maxTotalHitsSize represents the total number of docs to
  ...
  const maxTotalHitsSize = Math.min(totalHits, ruleParams.maxSignals); // number of docs in the current search result
