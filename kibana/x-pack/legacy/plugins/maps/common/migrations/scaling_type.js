...
function isEsDocumentSource(layerDescriptor) {
  ...
  return sourceType === _constants.ES_SEARCH;
}
...
function migrateUseTopHitsToScalingType({
  ...
  layerList.forEach(layerDescriptor => {
    if (isEsDocumentSource(layerDescriptor)) {
      ...
      sourceDescriptor.scalingType = _lodash.default.get(layerDescriptor, 'sourceDescriptor.useTopHits', false) ? _constants.SCALING_TYPES.TOP_HITS : _constants.SCALING_TYPES.LIMIT; // @ts-ignore useTopHits no longer in type definition but that does not mean its not in live data
