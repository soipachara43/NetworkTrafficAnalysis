...
const REPL_PATH = (0, _path.resolve)(__dirname, '../repl');
const CAN_REPL = canRequire(REPL_PATH); // xpack is installed in both dev and the distributable, it's optional if
// install is a link to the source, not an actual install

const XPACK_DIR = (0, _path.resolve)(__dirname, '../../../x-pack');
...
function applyConfigOverrides(rawConfig, opts, extraCliOptions) {
  const set = _lodash.default.partial(_lodash.default.set, rawConfig);
  ...
  set('plugins.paths', _lodash.default.compact([].concat(get('plugins.paths'), opts.pluginPath, XPACK_INSTALLED && !opts.oss ? [XPACK_DIR] : [])));
...
function _default(program) {
  ...
  command.action(async function (opts) {
    if (opts.dev && opts.devConfig !== false) {
      try {
        const kbnDevConfig = (0, _utils2.fromRoot)('config/kibana.dev.yml');
