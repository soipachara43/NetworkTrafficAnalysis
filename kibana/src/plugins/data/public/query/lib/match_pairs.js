 ...
 */
var pairs = ['()', '[]', '{}', "''", '""'];
var openers = pairs.map(function (pair) {
  return pair[0];
});
var closers = pairs.map(function (pair) {
  return pair[1];
});

function matchPairs(_ref) {
  ...
  if (shouldMoveCursorForward(key, value, selectionStart, selectionEnd)) {
    ...
    preventDefault();
    var newValue = value.substr(0, selectionStart) + key + value.substring(selectionStart, selectionEnd) + closers[openers.indexOf(key)] + value.substr(selectionEnd);
    ...
    var _newValue = value.substr(0, selectionEnd - 1) + value.substr(selectionEnd + 1);

    updateQuery(_newValue, selectionStart - 1, selectionEnd - 1);
  }
}
...
function shouldInsertMatchingCloser(key, value, selectionStart, selectionEnd) {
  ...
  var followingCharacter = value.charAt(selectionStart + 1); // Don't insert if the preceding character is a backslash
