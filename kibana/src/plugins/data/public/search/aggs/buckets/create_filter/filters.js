 ...
 * under the License.
 */
var createFilterFilters = function createFilterFilters(aggConfig, key) {
  // have the aggConfig write agg dsl params
  var dslFilters = (0, _lodash.get)(aggConfig.toDsl(), 'filters.filters');
  var filter = dslFilters[key];
  var indexPattern = aggConfig.getIndexPattern();

  if (filter && indexPattern && indexPattern.id) {
    return (0, _common.buildQueryFilter)(filter.query, indexPattern.id, key);
  }
};
