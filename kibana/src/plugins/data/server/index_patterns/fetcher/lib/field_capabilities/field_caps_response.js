...
function readFieldCapsResponse(fieldCapsResponse) {
  ...
  const kibanaFormattedCaps = Object.keys(capsByNameThenType).map(fieldName => {
    ...
    const uniqueKibanaTypes = (0, _lodash.uniq)(types.map(_common.castEsToKbnFieldTypeName));
    ...
    if (uniqueKibanaTypes.length > 1) {
      return {
        ...
        conflictDescriptions: types.reduce((acc, esType) => ({ ...acc,
    ...
    return {
      name: fieldName,
      type: (0, _common.castEsToKbnFieldTypeName)(esType),
      esTypes: types,
      searchable: isSearchable,
      aggregatable: isAggregatable,
