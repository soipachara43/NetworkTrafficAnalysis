...
function expandLiteralStrings(data) {
  return data.replace(LITERAL_STRING_CANDIDATES, (match, string) => {
    ...
    if (string.match(/\\./)) {
      ...
      const lastDoubleQuoteIdx = string.lastIndexOf('"'); // Handle a special case where we may have a value like "\"test\"". We don't
      ...
      const rawStringifiedValue = string.slice(firstDoubleQuoteIdx, string.length); // Remove one level of JSON stringification
