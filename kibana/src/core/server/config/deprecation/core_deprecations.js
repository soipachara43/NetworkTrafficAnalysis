...
const xsrfDeprecation = (settings, fromPath, log) => {
  if ((0, _lodash.has)(settings, 'server.xsrf.whitelist') && (0, _lodash.get)(settings, 'server.xsrf.whitelist').length > 0) {
    log('It is not recommended to disable xsrf protections for API endpoints via [server.xsrf.whitelist]. ' + 'It will be removed in 8.0 release. Instead, supply the "kbn-xsrf" header.');
  }
