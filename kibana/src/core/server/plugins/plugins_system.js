...
class PluginsSystem {
  ...
  getPluginDependencies() {
    ...
    return new Map([...this.plugins].map(([name, plugin]) => [plugin.opaqueId, [...new Set([...plugin.requiredPlugins, ...plugin.optionalPlugins.filter(optPlugin => this.plugins.has(optPlugin))])].map(depId => this.plugins.get(depId).opaqueId)]));
  }
  ...
  async setupPlugins(deps) {
    ...
    for (const pluginName of sortedPlugins) {
      ...
      const contract = await (0, _utils.withTimeout)({
