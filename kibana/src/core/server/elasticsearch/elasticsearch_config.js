...
const deprecations = () => [(settings, fromPath, log) => {
  ...
  if (es.username === 'elastic') {
    log(`Setting [${fromPath}.username] to "elastic" is deprecated. You should use the "kibana" user instead.`);
  }
...
const readKeyAndCerts = rawConfig => {
  ...
  if ((_rawConfig$ssl$keysto = rawConfig.ssl.keystore) === null || _rawConfig$ssl$keysto === void 0 ? void 0 : _rawConfig$ssl$keysto.path) {
  ...
  if ((_rawConfig$ssl$trusts = rawConfig.ssl.truststore) === null || _rawConfig$ssl$trusts === void 0 ? void 0 : _rawConfig$ssl$trusts.path) {
