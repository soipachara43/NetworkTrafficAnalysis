...
const callAPI = async (client, endpoint, clientParams = {}, options = {
  ...
  try {
    ...
    throw _errors.ElasticsearchErrorHelpers.decorateNotAuthorizedError(err);
  }
};
/**
...
class ClusterClient {
   ...
   */
  constructor(config, log, getAuthHeaders = noop) {
    this.config = config;
    this.log = log;
  ...
  asScoped(request) {
    ...
    return new _scoped_cluster_client.ScopedClusterClient(this.callAsInternalUser, this.callAsCurrentUser, (0, _router.filterHeaders)(this.getHeaders(request), this.config.requestHeadersWhitelist));
  }
  /**
   * Calls specified endpoint with provided clientParams on behalf of the
  ...
  getHeaders(request) {
    ...
    const headers = (0, _router.ensureRawRequest)(request).headers;
    return { ...headers,
      ...authHeaders
    };
  }
