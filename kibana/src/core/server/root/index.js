...
class Root {
  constructor(rawConfigProvider, env, onShutdown) {
    ...
    _defineProperty(this, "loggingService", void 0);

    _defineProperty(this, "server", void 0);

    _defineProperty(this, "loggingConfigSubscription", void 0);

    this.loggingService = new _logging.LoggingService();
    this.logger = this.loggingService.asLoggerFactory();
    this.log = this.logger.get('root');
    this.server = new _server.Server(rawConfigProvider, env, this.logger);
  }
  ...
  async setupLogging() {
    ...
    (0, _operators.switchMap)(() => configService.atPath('logging')), (0, _operators.map)(config => this.loggingService.upgrade(config)), // This specifically console.logs because we were not able to configure the logger.
