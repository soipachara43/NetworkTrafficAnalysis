...
async function getNonExistingReferenceAsKeys(savedObjects, savedObjectsClient, namespace) {
...
async function validateReferences(savedObjects, savedObjectsClient, namespace) {
  const errorMap = {};
  const nonExistingReferenceKeys = await getNonExistingReferenceAsKeys(savedObjects, savedObjectsClient, namespace); // Filter out objects with missing references, add to error object

  let filteredObjects = savedObjects.filter(savedObject => {
    const missingReferences = [];
    const enforcedTypeReferences = (savedObject.references || []).filter(filterReferencesToValidate);
