...
function reader(callCluster, index, {
  batchSize = 10,
  scrollDuration = '15m'
}) {
  ...
  const nextBatch = () => scrollId !== undefined ? callCluster('scroll', {
    scroll,
    scrollId
  }) : callCluster('search', {
    body: {
  ...
  const close = async () => scrollId && (await callCluster('clearScroll', {
    scrollId
  }));
...
async function write(callCluster, index, docs) {
  ...
  const err = _lodash.default.find(result.items, 'index.error.reason');
...
async function migrationsUpToDate(callCluster, index, migrationVersion, retryCount = 10) {
  try {
    ...
    if (!_lodash.default.get(indexInfo, 'mappings.properties.migrationVersion')) {
      return false;
...
async function convertToAlias(callCluster, info, alias, batchSize, script) {
  await callCluster('indices.create', {
    body: {
  ...
  await claimAlias(callCluster, info.indexName, alias, [{
    remove_index: {
      index: alias
    }
  }]);
...
async function claimAlias(callCluster, index, alias, aliasActions = []) {
  ...
  await callCluster('indices.refresh', {
    index
  });
}
/**
...
async function reindex(callCluster, source, dest, batchSize, script) {
