...
async function importDashboards(req) {
  ...
  const savedObjectsClient = req.getSavedObjectsClient(); // The server assumes that documents with no migrationVersion are up to date.
  ...
  const docs = payload.objects.filter(item => !exclude.includes(item.type)).map(doc => ({ ...doc,
