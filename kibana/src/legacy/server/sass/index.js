...
async function sassMixin(kbnServer, server, config) {
  ...
  try {
    scssBundles = await buildAll({
      ...
      log,
      buildDir: (0, _utils2.fromRoot)('built_assets/css')
    });
  ...
  watcher.on('all', async (event, path) => {
    ...
    server.log(['debug', 'scss'], `${path} triggered ${event}`); // build bundles containing the changed file
