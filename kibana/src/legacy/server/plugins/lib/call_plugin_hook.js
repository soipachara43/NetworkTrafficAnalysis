...
async function callPluginHook(hookName, plugins, id, history) {
  const plugin = plugins.find(plugin => plugin.id === id); // make sure this is a valid plugin id
  ...
  const path = [...history, id]; // make sure we are not trying to load a dependency within itself
