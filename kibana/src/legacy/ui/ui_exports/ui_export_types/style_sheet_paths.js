...
exports.styleSheetPaths = void 0;
...
function getPublicPath(pluginSpec, localPath) {
  ...
  let relativePath = _path.default.relative(pluginSpec.getPublicDir(), localPath); // replace back slashes on windows
...
function getStyleSheetPath(pluginSpec, localPath, theme) {
  const extname = _path.default.extname(localPath);
...
function normalize(localPath, type, pluginSpec) {
  ...
  const publicDir = _path.default.normalize(pluginSpec.getPublicDir());
...
const styleSheetPaths = (0, _modify_reduce.wrap)((0, _modify_reduce.mapSpec)(normalize), _reduce.flatConcatAtType);
exports.styleSheetPaths = styleSheetPaths;