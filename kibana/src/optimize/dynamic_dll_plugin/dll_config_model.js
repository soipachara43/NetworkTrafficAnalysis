...
function generateDLL(config) {
  ...
  return {
    ...
    plugins: [new _webpack.default.DllPlugin({
      context: dllContext,
      name: dllBundleName,
...
function extendRawConfig(rawConfig) {
  ...
  const threadLoaderPoolConfig = rawConfig.threadLoaderPoolConfig; // Create webpack entry object key with the provided dllEntryName

  dllChunks.reduce((dllEntryObj, chunk) => {
    dllEntryObj[`${dllEntryName}${chunk}`] = [`${dllOutputPath}/${dllEntryName}${chunk}${dllEntryExt}`];
    return dllEntryObj;
  }, dllEntry); // Export dll config map
...
function common(config) {
  return (0, _webpackMerge.default)(generateDLL(config));
}
...
function configModel(rawConfig = {}) {
  ...
  if (config.dllDevMode) {
    return (0, _webpackMerge.default)(common(config), unoptimized());
  }

  return (0, _webpackMerge.default)(common(config), optimized());
}