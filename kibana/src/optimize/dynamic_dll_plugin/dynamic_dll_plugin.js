...
var _RawModule = _interopRequireDefault(require("webpack/lib/RawModule"));
 ...
 * under the License.
 */
const realPathAsync = (0, _util.promisify)(_fs.default.realpath);
...
class DynamicDllPlugin {
  constructor({
    ...
    this.dllCompiler = new _dll_compiler.DllCompiler(uiBundles, threadLoaderPoolConfig, logWithMetadata);
    this.entryPaths = (0, _dll_entry_template.dllEntryTemplate)();
    this.afterCompilationEntryPaths = (0, _dll_entry_template.dllEntryTemplate)();
  ...
  apply(compiler) {
    ...
    this.registerInitBasicHooks(compiler); // The dll reference should always be bind to the
    // main webpack config.

    this.bindDllReferencePlugin(compiler); // Verify if we must init and run the dynamic dll plugin tasks.
  ...
  registerBeforeCompileHook(compiler) {
    compiler.hooks.beforeCompile.tapPromise('DynamicDllPlugin', async ({
      normalModuleFactory
    }) => {
      normalModuleFactory.hooks.factory.tap('DynamicDllPlugin', actualFactory => (params, cb) => {
        ...
        normalModuleFactory.cachePredicate = module => !(module.stubType === DLL_ENTRY_STUB_MODULE_TYPE); // Overrides the normalModuleFactory module creation behaviour
      ...
      compilation.hooks.needAdditionalPass.tap('DynamicDllPlugin', () => {
        ...
        for (const module of compilation.modules) {
          ...
          if (module.delegateData) {
            const absoluteResource = _path.default.resolve(dllContext, module.userRequest);
            ...
            if (absoluteResource.includes('node_modules') || absoluteResource.includes('webpackShims')) {
              ...
              requiresMap[(0, _normalizePath.default)(_path.default.relative(dllOutputPath, absoluteResource))] = true;
            }
          } // include requires for modules that need to be added to the dll


          if (module.stubType === DLL_ENTRY_STUB_MODULE_TYPE) {
            requiresMap[(0, _normalizePath.default)(_path.default.relative(dllOutputPath, module.stubResource))] = true;
          }
        } // Sort and join all the discovered require deps
        // in order to create a consistent entry file


        this.afterCompilationEntryPaths = (0, _dll_entry_template.dllEntryTemplate)(Object.keys(requiresMap)); // The dll compilation will run if on of the following conditions return true:
        ...
        compilation.needsDLLCompilation = this.afterCompilationEntryPaths !== this.entryPaths || !this.dllCompiler.dllsExistsSync() || this.isToForceDLLCreation() && this.performedCompilations === 0;
        this.entryPaths = this.afterCompilationEntryPaths; // Only run this info log in the first performed dll compilation
        // per each execution run

        if (this.performedCompilations === 0) {
          this.logWithMetadata(['info', 'optimize:dynamic_dll_plugin'], compilation.needsDLLCompilation ? 'Need to compile the client vendors dll' : 'No need to compile client vendors dll');
        }
  ...
  registerDoneHook(compiler) {
    compiler.hooks.done.tapPromise('DynamicDllPlugin', async stats => {
      ...
      this.performedCompilations = 0; // reset this flag var set from the node env FORCE_DLL_CREATION on init,
  ...
  async mapNormalModule(module) {
    ...
    if ((0, _dll_allowed_modules.notInNodeModulesOrWebpackShims)((await realPathAsync(module.resource)))) {
      return;
    }

    const dirs = module.resource.split(_path.default.sep);
    const nodeModuleName = dirs[dirs.lastIndexOf('node_modules') + 1]; // ignore webpack loader modules
    ...
    if ((0, _dll_allowed_modules.inPluginNodeModules)(module.resource)) {
      return;
    } // also ignore files that are symlinked into plugins node_modules, but only
    // do the `realpath` call after checking the plain resource path


    if ((0, _dll_allowed_modules.inPluginNodeModules)((await realPathAsync(module.resource)))) {
      return;
    ...
    const stubModule = new _RawModule.default(`/* pending dll entry */`, `dll pending:${module.resource}`, module.resource);
  ...
  async runDLLCompiler(mainCompiler) {
    ...
    this.dllCompiler.getManifestPaths().forEach(chunkDllManifestPath => mainCompiler.inputFileSystem.purge(chunkDllManifestPath));
