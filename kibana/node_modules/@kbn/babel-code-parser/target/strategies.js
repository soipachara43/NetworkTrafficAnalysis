...
async function dependenciesParseStrategy(cwd, parseSingleFile, mainEntry, wasParsed, results) {
  ...
  const dependencies = (await parseSingleFile(mainEntry, _visitors.dependenciesVisitorsGenerator)).filter(dep => !_module.builtinModules.includes(dep)); // Return the list of all the new entries found into
  // the current mainEntry that we could use to look for
  // new dependencies

  return dependencies.reduce((filteredEntries, entry) => {
    const absEntryPath = (0, _path.resolve)(cwd, (0, _path.dirname)(mainEntry), entry); // NOTE: cwd for following canRequires is absEntryPath
    // because we should start looking from there

    const requiredPath = (0, _can_require.canRequire)(absEntryPath, absEntryPath);
    const requiredRelativePath = (0, _can_require.canRequire)(entry, absEntryPath);
